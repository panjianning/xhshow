...
while has_more:
    ...
    res = requests.get(uri, headers={**headers, 'User-Agent': ua}, cookies=cookies, params=params)
    data = res.json()
    ...
    print(f"第 {page} 页抓取完成，当前共 {len(all_comments)} 条，has_more={has_more}")
    
    if has_more:
        page += 1
        time.sleep(1.5)  # 避免请求过快被风控
...
# 打印结果
print("\n" + "="*50)
print(f"总计获取评论：{len(all_comments)} 条 (不包含子评论的展开)")
print("="*50)
